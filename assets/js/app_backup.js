...
class PasswordProtection {
    ...
    resetApp() {
        ...
        // Rensa alla textinput-fält
        const textInputs = [
            'customer-company', 'customer-contact', 'customer-address', 'customer-phone', 
            'customer-email', 'customer-city', 'customer-postal-code'
        ];
        ...
        // Rensa alla select-fält för elinstallationer
        const selectInputs = [
            'preferred-day', 'preferred-time', 'materialkostnad'
        ];
        ...
        // Rensa textarea-fält
        const textareas = ['arb-beskrivning', 'tillagg-service-type', 'tillagg-service-comment'];
        ...
        // Rensa radio button-grupper för elinstallationer
        const radioGroups = ['fastighet_rot_berättigad', 'är_du_berättigad_rot_avdrag', 'delat_rot_avdrag'];
        radioGroups.forEach(groupName => {
            const radios = document.querySelectorAll(`input[name="${groupName}"]`);
            radios.forEach(radio => radio.checked = false);
        });
        
        // Dölj ROT-relaterade sektioner
    ...
    resetAllPriceDisplays() {
        const priceIds = [
            'subtotal-price-display', 'total-with-vat', 'final-customer-price',
            'rot-deduction', 'material-deduction'
        ];
        ...
        // Dölj ROT-relaterade rader
        const priceDisplays = ['rot-row', 'material-row'];
...
class QuoteCalculator {
    ...
    updatePricingDisplay(totalExclVat, vatAmount, totalInclVat, rutDeduction, finalTotal, extras = {}) {
        ...
        // Update ROT deduction if applicable
        ...
        if (rutDeduction > 0) {
            if (rotDeductionEl) {
                rotDeductionEl.textContent = '-' + new Intl.NumberFormat('sv-SE').format(Math.round(rutDeduction)) + ' kr';
            }
            if (rotRow) {
    ...
    // Metoder för elinstallationer
    populateDropdowns() {
        console.log('🔧 Populerar dropdown-menyer för elinstallationer');
        // Elinstallationer använder statiska dropdown-menyer som definieras i HTML
    }
